...
private {
    ...
    import polyplex.math.util : is_vector, is_quaternion, is_matrix;

    version(unittest) {
        import polyplex.math.linalg : vec2, vec2i, vec3, vec3i, quat;
    }
}
...
import linalg = polyplex.math.linalg;
import aabb = polyplex.math.aabb;
import util = polyplex.math.util;
import rect = polyplex.math.rectangle;
// Polyplex aliases.
alias linalg.Vector!(float, 2) Vector2;
alias linalg.Vector!(float, 3) Vector3;
alias linalg.Vector!(float, 4) Vector4;
alias linalg.Vector!(double, 2) Vector2d;
alias linalg.Vector!(double, 3) Vector3d;
alias linalg.Vector!(double, 4) Vector4d;
...
alias linalg.Matrix!(float, 3, 4) Matrix3x4;
alias linalg.Matrix!(float, 4, 4) Matrix4x4;

alias linalg.Quaternion!(float) Quaternion;